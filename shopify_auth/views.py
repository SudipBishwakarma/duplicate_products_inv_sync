...
def _return_address(request):
    return request.session.get('return_to') or reverse('app:index')
...
def finalize(request):
    ...
    try:
        ...
        return redirect(reverse('shopify_auth:login'))
...
#
# @csrf_exempt
# def webhook_product_update(request):
...
#             task_webhook_product_update(data, verbose_name=f'Task for product/update webhook event: {shop_url}')
#     return render(request, 'shopify_auth/webhook.html')
